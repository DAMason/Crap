...
// $Id: CrapLLAnalysis.h,v 1.8 2013/05/05 12:01:32 yiiyama Exp $
//

#ifndef CrapLLAnalysis_h
#define CrapLLAnalysis_h
...
class CrapLLAnalysis {
public:
  CrapLLAnalysis(TTree&);
  virtual ~CrapLLAnalysis();
...
CrapLLAnalysis::CrapLLAnalysis(TTree& tree) :
  event(),
  fTree(&tree),
  outputName("analysis"),
...
CrapLLAnalysis::~CrapLLAnalysis()
{
}

void
CrapLLAnalysis::IncludeAJson(TString const& _fileName)
{
  if(_fileName == "") return;
...
bool
CrapLLAnalysis::IsGoodLumi(UInt_t run, UInt_t lumi) const
{
...
bool
CrapLLAnalysis::PassTriggers() const
{
